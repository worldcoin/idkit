...
describe("getSessionCommitment", () => {
	describe("validation", () => {
		it("rejects missing session_ prefix", () => {
			expect(() => getSessionCommitment("0".repeat(128))).toThrow("Invalid session ID");
		});

		it("rejects wrong hex length (too short)", () => {
			expect(() => getSessionCommitment("session_" + "0".repeat(64))).toThrow("Invalid session ID");
		});

		it("rejects wrong hex length (too long)", () => {
			expect(() => getSessionCommitment("session_" + "0".repeat(130))).toThrow("Invalid session ID");
		});

		it("rejects non-hex characters", () => {
			expect(() => getSessionCommitment("session_" + "g".repeat(128))).toThrow("Invalid session ID");
		});
	...
	describe("commitment extraction", () => {
		...
		it("extracts all-ff commitment", () => {
			...
			expect(getSessionCommitment(sessionId)).toBe(BigInt("0x" + FF_COMMITMENT));
		});
		...
		it("ignores oprf_seed bytes (second half)", () => {
			...
			expect(getSessionCommitment(sessionA)).toBe(getSessionCommitment(sessionB));
		});

		it("handles uppercase hex", () => {
			const sessionId = `session_${"A".repeat(64)}${"B".repeat(64)}`;
			expect(getSessionCommitment(sessionId)).toBe(BigInt("0x" + "A".repeat(64)));
		});

		it("handles mixed-case hex", () => {
			const sessionId = `session_${"aB".repeat(32)}${"Cd".repeat(32)}`;
			expect(getSessionCommitment(sessionId)).toBe(BigInt("0x" + "aB".repeat(32)));
		});
