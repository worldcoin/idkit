...
/**
 * Returns the first successful response item from an IDKitResult
 */
export function firstSuccessful(
  result: IDKitResult,
): IDKitResponseItem | undefined {
  return result.responses.find((r) => r.proof_data !== undefined);
}

/**
 * Returns all successful response items from an IDKitResult
 */
export function allSuccessful(result: IDKitResult): IDKitResponseItem[] {
  return result.responses.filter((r) => r.proof_data !== undefined);
}

/**
 * Returns true if all responses are successful
 */
export function isAllSuccessful(result: IDKitResult): boolean {
  return (
    result.responses.length > 0 &&
    result.responses.every((r) => r.proof_data !== undefined)
  );
}
 ...
 */
export function hasAnySuccessful(result: IDKitResult): boolean {
  return result.responses.some((r) => r.proof_data !== undefined);
}

/**
 * Returns the count of successful responses
 */
export function successCount(result: IDKitResult): number {
  return result.responses.filter((r) => r.proof_data !== undefined).length;
}

/**
 * Returns the count of failed responses
 */
export function failureCount(result: IDKitResult): number {
  return result.responses.filter((r) => r.error !== undefined).length;
}
